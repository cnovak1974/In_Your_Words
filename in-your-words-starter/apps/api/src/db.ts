...
async function mockQuery(sql: string, values: any[] = []) {
  ...
  else if (q.includes("from turns") && q.includes("intent='story_answer'")) rows=[...turns.values()].filter(t=>t.session_id===values[0]&&t.id!==values[1]&&t.intent==="story_answer"&&t.transcript).sort((a,b)=>b.created_at-a.created_at).slice(0,50).map(t=>({...t,chronology_status:t.ai_payload?.chronology_status,current_life_period:t.ai_payload?.current_life_period,current_topic:t.ai_payload?.current_topic,story_thread:t.ai_payload?.story_thread,story_is_emerging:t.ai_payload?.story_is_emerging == null ? null : String(t.ai_payload.story_is_emerging)}));
