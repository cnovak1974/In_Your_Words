...
async function mockQuery(sql: string, values: any[] = []) {
  ...
  else if (q.includes("ai_payload->'newsreel_candidate'") && q.includes("from turns") && q.includes("status='complete'")) rows=[...turns.values()].filter(t=>t.session_id===values[0]&&t.status==="complete"&&t.ai_payload?.newsreel_candidate).sort((a,b)=>b.created_at-a.created_at).slice(0,1).map(t=>({newsreel_candidate:t.ai_payload.newsreel_candidate}));
