...
| Raw-audio storage | Mock upload route writes audio bytes to a process-local `Map`; live code path already uses S3-compatible presigned PUT/GET and HEAD | Mock/in-memory in the tested flow; live adapter unverified | `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, bucket CORS | First real integration: upload to private R2, HEAD-confirm, retrieve, and play the exact object in a two-turn iPad test |
...
| Secret hygiene | No `.gitignore` exists in the current repository; `.env.example` contains placeholders only | Not ready for credentials | Root `.gitignore` covering `.env`, `.env.*` except examples, credential files, and local tunnel artifacts | Add and verify ignore rules before creating any real credential file; confirm with `git check-ignore` |
